...
export class SpotlightService extends BaseService {
  ...
  public async submitSpotlight(
    payload: ISpotlightSubmissionPayload
  ): Promise<IServiceResponse<ISpotlightDocument>> {
    try {
      // Check if email already exists
      const existingSpotlight = await this.spotlightRepository.findByEmail(payload.email);
      if (existingSpotlight) {
        return this.createErrorResponse(
          'Email already submitted for spotlight',
  ...
  public async getSpotlightUploadUrl(
    email: string,
    fileType: string
    ...
    try {
      ...
      if (!spotlight) {
        return this.createErrorResponse(
          'Please submit your spotlight information first',
          HttpStatusCode.BAD_REQUEST
        );
      }
      ...
      await this.spotlightRepository.updateById(spotlight._id.toString(), {
