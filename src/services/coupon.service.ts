...
export class CouponService extends BaseService {
  ...
  public async createCoupon(
    ...
    try {
      ...
      const existingCoupon = await this.couponRepository.findByCouponCode(
        couponData.coupon as string
      );
      if (existingCoupon) {
    ...
    try {
      ...
      if (couponData.coupon) {
        ...
        if (couponData.coupon !== existingCoupon.coupon) {
          const couponWithSameCode = await this.couponRepository.findByCouponCode(
            couponData.coupon
          );
