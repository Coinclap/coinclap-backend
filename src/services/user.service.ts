...
export class UserService extends BaseService {
  ...
  public async verifyEmailOtp(
    userId: string,
    otp: string
  ): Promise<IServiceResponse<IAuthResponse>> {
    try {
      // Verify OTP
  ...
  ): Promise<IServiceResponse<IUserDocument>> {
    try {
  ...
  public async isUsernameAvailable(username: string): Promise<boolean> {
    try {
      ...
      if (this.redis.isReady()) {
        await redisClient.set(`${CacheKeys.USERNAME_PREFIX}${username}`, isAvailable.toString(), {
          EX: 3600,
        });
      }
  ...
  public async loginUser(loginData: ILoginData): Promise<IServiceResponse<IAuthResponse>> {
    try {
      ...
      // Get password
      const passwordDoc = await this.passwordRepository.findByUserIdWithPassword(
        user._id.toString()
      );
      if (!passwordDoc) {
        return this.createErrorResponse('Invalid credentials', HttpStatusCode.UNAUTHORIZED);
  ...
  public async forgotPassword(email: string): Promise<IServiceResponse> {
    try {
      ...
      await this.forgotPasswordRepository.createForgotPasswordRequest(
        user._id.toString(),
        user.email,
        otp,
        otpExpiry
  ...
  public async resetPassword(resetToken: string, newPassword: string): Promise<IServiceResponse> {
    try {
      ...
      const forgotPasswordRequest = await this.forgotPasswordRepository.findValidRequestByEmail(
        decoded.email
      );
      if (
        ...
        return this.createErrorResponse(
          'Reset token has been used or expired',
          HttpStatusCode.BAD_REQUEST
        );
      }
