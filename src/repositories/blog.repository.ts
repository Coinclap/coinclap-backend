...
export class BlogRepository extends BaseRepository<IBlogDocument> {
  ...
  public async updateBlog(id: string, blogData: Partial<IBlogDocument>): Promise<IBlogDocument | null> {
  ...
  public async publishBlog(id: string): Promise<IBlogDocument | null> {
    return await this.model
      .findByIdAndUpdate(id, { isPublished: true, updatedAt: Math.floor(Date.now() / 1000) }, { new: true })
      .exec()
  }

  public async unpublishBlog(id: string): Promise<IBlogDocument | null> {
    return await this.model
      .findByIdAndUpdate(id, { isPublished: false, updatedAt: Math.floor(Date.now() / 1000) }, { new: true })
      .exec()
  }
}
